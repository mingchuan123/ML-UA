...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing == True:
            ...
            #self.epsilon = float(1/self.trail*self.trail)
            self.epsilon = math.exp(-0.05*self.trail)
            self.trail += 1
            self.alpha = 0.5
            #self.epsilon -= 0.5
    ...
    def build_state(self):
        ...
        state = tuple((inputs['waypoint'],inputs['light'],inputs['oncoming']))
    ...
    def get_maxQ(self, state):
        ...
        maxQ = 0
        #maxaction = self.valid_actions[0]
    ...
    def createQ(self, state):
        ...
        if self.learning == True:
            if state not in self.Q:
                #self.Q = defaultdict(dict)
                #self.Q[state][self.valid_actions] = 0.0
                #state = dict()
    ...
    def choose_action(self, state):
        ...
        #max_val = get_maxQ(state)
        
        if self.learning == False:
            best_action = random.choice(self.valid_actions)
        else:
            maxQ = self.get_maxQ(state)
            #actionQ = random_choice([action for action in self.valid_actions if self.Q[state][action] == maxQ]
            actionQ = random.choice([action for action in self.valid_actions if self.Q[state][action] == maxQ])
            best_action = np.random.choice([random.choice(self.valid_actions),actionQ],p=[self.epsilon, 1-self.epsilon])
            #max_val, best_action = self.get_maxQ(state)
            #for action in self.valid_actions:
                #opt_val = self.Q[state][action]
                #if opt_val > max_val:
                    #max_val = opt_val
                    #best_action = action
                #elif opt_val == max_val:
